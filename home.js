...
//CODE HERE
const newDeals = deals.map(function(obj){

    if(obj.title === '15% Off!'){
        return (obj.title).replace('15%','10%')
    }
...
const anotherMonth = deals.map(function(obj){
if(obj.title === 'Free Kids Meal with 2 Regular Entrees'){
    return (obj.desc).replace('March','April').replace(/\s/g, '')
}
return obj
})
